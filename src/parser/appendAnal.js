 ...
 * @property {{c: number, h:number, n: number, s: number}} general.spectra[].elementalAnalysis - is the parsed elemental analysis (if provided).
 */
/**
 ...
 * @param {String} text - is the full text-source of the synthesis/determination found in the literature.
 * @param {Object} options - is a debug option to add the actual recognized and parsed text in a property called source or control.
 * @param {Boolean} [options.debug = false] - when 'true' triggers the debugging option.
 * @return {result} - returns the same input object parameters with append element analysis data (if provided).
 */
...
export default function appendAnal(result, text, options = {}) {
  ...
  if (text.match(AnalPattern)) {
    ...
    // to use for checking the determination's accuracy.
